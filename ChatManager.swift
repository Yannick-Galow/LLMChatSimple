...
class ChatManager: ObservableObject {
    ...
    func sendMessage(_ content: String) async {
        ...
        do {
            ...
            var errorMessage = "Entschuldigung, es ist ein Fehler aufgetreten: \(error.localizedDescription)"
            
            // Spezifische Fehlermeldungen für häufige Probleme
            if let apiError = error as? APIError {
                switch apiError {
                case .serverError(404):
                    errorMessage = "❌ Server nicht gefunden (404)\n\nMögliche Lösungen:\n• Server-IP überprüfen\n• Server starten\n• Port 1010 freigeben"
                case .serverError(500):
                    errorMessage = "❌ Server-Fehler (500)\n\nDer Server hat einen internen Fehler."
                case .networkError(_):
                    errorMessage = "❌ Netzwerkfehler\n\n• Internetverbindung prüfen\n• Server-IP überprüfen\n• Firewall-Einstellungen prüfen"
                case .invalidURL:
                    errorMessage = "❌ Ungültige URL\n\nServer-URL ist fehlerhaft konfiguriert."
                default:
                    break
                }
            }
            
            let errorMsg = Message(role: .assistant, content: errorMessage)
            conversation.addMessage(errorMsg)
            updateCurrentConversation(conversation)
        }
    ...
    // Debug-Funktion um Server-URL anzuzeigen
    func getServerURL() -> String {
        return openAIAPI.baseURL
    }
    
    // Funktion um Server-URL zu ändern
    func updateServerURL(_ newIP: String) {
        UserDefaults.standard.set(newIP, forKey: "llm_server_ip")
    }
    
    // Funktion um Server-Protokoll zu ändern
    func updateServerProtocol(_ protocol: String) {
        UserDefaults.standard.set(`protocol`, forKey: "llm_server_protocol")
    }
    
    // Funktion um das aktuelle Protokoll zu erhalten
    func getServerProtocol() -> String {
        return UserDefaults.standard.string(forKey: "llm_server_protocol") ?? "http"
    }
}
...
class OpenAIAPI {
    ...
    private var baseURL: String {
        let savedHost = UserDefaults.standard.string(forKey: "llm_server_ip") ?? "192.168.0.234:1010"
        let protocol = UserDefaults.standard.string(forKey: "llm_server_protocol") ?? "http"
        return "\(protocol)://\(savedHost)/v1/chat/completions"
    }
